"""
NorthStar Brands — CPG synthetic data generator
================================================
Generation plan (Tier 1 + UC write, Polars/NumPy → Databricks Connect bridge, seed=42):

Target: REPLACE_WITH_CATALOG.northstar_cpg  (workspace: YOUR_WORKSPACE)

STRUCTURED (Genie):
  products        ~80 SKUs  (curated 13 base products × pack-size variants)
  retailers       10 accounts across channels (Mass/Grocery/Club/Drug/Convenience/eComm)
  sales_facts     product × retailer × week (104 wks) — sell-in (shipments) + sell-out (POS)
  inventory       weekly on-hand + weeks-of-supply by product × retailer
  trade_promotions promo events with spend / lift / incremental units / ROI (some negative)
  distribution    weekly ACV %, stores selling
  market_share    monthly $ sales + category share by brand × category

UNSTRUCTURED (Vector Search — Delta table `documents` with text column):
  product_spec, consumer_review, brand_guideline, promo_playbook, competitive_brief

Run:
  DATABRICKS_CONFIG_PROFILE=YOUR_WORKSPACE \
  uv run --with polars --with numpy --with "databricks-connect>=16.4,<17.0" \
    data_generation/generate_cpg_data.py
"""
import numpy as np
import polars as pl
...
# ---------------------------------------------------------------------------
# base products: (name, brand, category, subcategory, base_price, cogs_pct, allergens, key_ingredients, claims)
BASE_PRODUCTS = [
...
# pack-size variants per category to expand SKU count to ~80
PACK_VARIANTS = {
...
# real (well-known) retailer names — clearly synthetic demo data
RETAILERS = [
    ("Walmart",     "Mass",         "National",  4700),
    ("Target",      "Mass",         "National",  1950),
    ("Kroger",      "Grocery",      "Midwest",   2750),
    ("Albertsons",  "Grocery",      "West",      2200),
    ("Publix",      "Grocery",      "Southeast", 1350),
    ("Costco",      "Club",         "National",   590),
    ("Walgreens",   "Drug",         "National",  8500),
    ("7-Eleven",    "Convenience",  "National", 13000),
    ("Amazon",      "eCommerce",    "National",     0),
    ("Whole Foods", "Grocery",      "National",   530),
]
...
for (name, brand, cat, subcat, base_price, cogs_pct, allergens, ingredients, claims) in BASE_PRODUCTS:
    for variant, mult in PACK_VARIANTS[cat]:
        ...
        prod_rows.append({
            ...
            "unit_cogs": round(list_price * cogs_pct, 2),
            "allergens": allergens,
            "key_ingredients": ingredients,
            "claims": claims,
            "launch_date": np.datetime64("2022-01-01")
                + int(rng.integers(0, 900)) * np.timedelta64(1, "D"),
        })
        pid += 1

products = pl.DataFrame(prod_rows)
N_PROD = products.height
print(f"products: {N_PROD} rows")

# ---------------------------------------------------------------------------
# retailers table
# ---------------------------------------------------------------------------
ret_rows = []
for i, (rname, channel, region, stores) in enumerate(RETAILERS, start=1):
    ret_rows.append({
        "retailer_id": i,
        "retailer_name": rname,
        "channel": channel,
...
retailers = pl.DataFrame(ret_rows)
N_RET = retailers.height
print(f"retailers: {N_RET} rows")

# ---------------------------------------------------------------------------
# sales_facts — product × retailer × week (weekly, 104 weeks ending ~2026-04)
# ---------------------------------------------------------------------------
WEEKS = 104
week0 = np.datetime64("2024-05-04")  # Saturday week-ending
...
# per-product base weekly velocity (units), scaled by retailer store count
prod_velocity = rng.gamma(3.0, 60.0, size=N_PROD)  # mean ~180
ret_id_arr = retailers["retailer_id"].to_numpy()
ret_scale = retailers["store_count"].to_numpy().astype(float)
ret_scale = np.where(ret_scale == 0, 1500, ret_scale)  # Amazon eComm baseline
ret_scale = ret_scale / ret_scale.mean()

# product × retailer distribution mask (~75% of combos carried)
...
for ri in range(N_RET):
    for pi in range(N_PROD):
        ...
        # seasonality: snacks peak in summer/holiday, beverages in summer
        for wi, wd in enumerate(week_dates):
            ...
            units_sold = max(0, int(base * seas * promo_lift * noise))  # sell-out (POS)
            avg_price = round(price_arr[pi] * (0.78 if on_promo else rng.uniform(0.95, 1.02)), 2)
            pos_rev = round(units_sold * avg_price, 2)
            # sell-in (shipments) leads sell-out, with replenishment noise
            units_shipped = max(0, int(units_sold * rng.uniform(0.9, 1.2)))
            ship_rev = round(units_shipped * price_arr[pi] * 0.62, 2)  # wholesale ~62% of list
            sf.append((
                int(pid_arr[pi]), int(ret_id_arr[ri]), str(wd),
                units_shipped, ship_rev, units_sold, pos_rev, avg_price, bool(on_promo),
            ))
            # inventory snapshot (sparser: every 4 weeks)
            if wi % 4 == 0:
                woh = max(0, int(units_sold * rng.uniform(2.0, 6.0)))
                wos = round(woh / max(units_sold, 1), 1)
                inv.append((int(pid_arr[pi]), int(ret_id_arr[ri]), str(wd), woh, wos))
        # distribution snapshot (monthly): ACV%
        for wi in range(0, WEEKS, 4):
            acv = round(min(99.5, rng.uniform(35, 95) + (10 if ret_scale[ri] > 1.2 else 0)), 1)
            stores_sell = int(ret_scale[ri] * acv * 30)
            dist.append((int(pid_arr[pi]), int(ret_id_arr[ri]), str(week_dates[wi]), acv, stores_sell))

sales_facts = pl.DataFrame(
    sf,
    schema=["product_id", "retailer_id", "week_ending", "units_shipped", "shipment_revenue",
            "units_sold", "pos_revenue", "avg_selling_price", "on_promo"],
    orient="row",
).with_columns(pl.col("week_ending").str.to_date())
sales_facts = sales_facts.with_row_index("sale_id", offset=1)
print(f"sales_facts: {sales_facts.height} rows")

inventory = pl.DataFrame(
    inv, schema=["product_id", "retailer_id", "week_ending", "units_on_hand", "weeks_of_supply"],
    orient="row",
).with_columns(pl.col("week_ending").str.to_date()).with_row_index("inventory_id", offset=1)
print(f"inventory: {inventory.height} rows")

distribution = pl.DataFrame(
    dist, schema=["product_id", "retailer_id", "week_ending", "acv_pct", "stores_selling"],
    orient="row",
).with_columns(pl.col("week_ending").str.to_date()).with_row_index("dist_id", offset=1)
print(f"distribution: {distribution.height} rows")

# ---------------------------------------------------------------------------
# trade_promotions — events with spend / lift / incremental / ROI (some negative)
# ---------------------------------------------------------------------------
PROMO_TYPES = ["TPR", "Display", "Feature", "Feature+Display", "BOGO"]
n_promos = 600
promo_rows = []
for k in range(1, n_promos + 1):
    pi = int(rng.integers(0, N_PROD))
    ri = int(rng.integers(0, N_RET))
    ...
    spend = round(promoted_units * price_arr[pi] * depth, 2)  # trade spend
    ...
    promo_rows.append((
        k, int(pid_arr[pi]), int(ret_id_arr[ri]), ptype, str(start), str(end),
        spend, round(depth * 100, 1), baseline, promoted_units, incr_units,
        round(lift_pct * 100, 1), roi,
    ))
trade_promotions = pl.DataFrame(
    promo_rows,
    schema=["promo_id", "product_id", "retailer_id", "promo_type", "start_date", "end_date",
...
print(f"trade_promotions: {trade_promotions.height} rows  (negative ROI: "
      f"{trade_promotions.filter(pl.col('roi') < 0).height})")

# ---------------------------------------------------------------------------
# market_share — monthly $ sales + share by brand × category
# ---------------------------------------------------------------------------
months = [np.datetime64("2024-05", "M") + int(m) for m in range(24)]
brand_cat = products.select(["brand", "category"]).unique().to_dicts()
# competitor universe sizes per category ($ per month, the whole category)
CAT_SIZE = {"Snacks": 4.2e7, "Beverages": 5.8e7, "Personal Care": 3.1e7}
ms_rows = []
sid = 1
for mo in months:
    for bc in brand_cat:
        # NorthStar brand $ sales: derive a stable-ish share with drift
        base_share = {"Summit": 0.085, "Trailhead": 0.052, "Crunch Peak": 0.071, "Orchard": 0.041,
                      ...
                      "PureLeaf": 0.078, "Evergreen": 0.055, "Lumen": 0.047}.get(bc["brand"], 0.05)
        drift = rng.normal(0, 0.004)
        share = max(0.005, base_share + drift)
        cat_dollars = CAT_SIZE[bc["category"]] * rng.uniform(0.95, 1.05)
        brand_dollars = round(cat_dollars * share, 2)
        ms_rows.append((sid, str(mo) + "-01", bc["brand"], bc["category"],
                        brand_dollars, round(cat_dollars, 2), round(share * 100, 2)))
        sid += 1
market_share = pl.DataFrame(
    ms_rows,
    schema=["share_id", "month", "brand", "category", "brand_dollar_sales",
            "category_dollar_sales", "dollar_share_pct"],
    orient="row",
).with_columns(pl.col("month").str.to_date())
print(f"market_share: {market_share.height} rows")
...
def add_doc(doc_type, title, brand, category, content):
    global did
    doc_rows.append({
        "doc_id": did, "doc_type": doc_type, "title": title, "brand": brand,
        "category": category,
...
for (name, brand, cat, subcat, base_price, cogs_pct, allergens, ingredients, claims) in BASE_PRODUCTS:
    ...
    add_doc("product_spec", f"{name} — Product Specification", brand, cat, f"""
        ...
        Allergen statement: {allergens}. Manufactured in a facility that also processes nuts and dairy.
        Product claims: {claims}.
        Suggested retail price (single unit): ${base_price:.2f}.
        Shelf life: {int(rng.integers(6, 18))} months from manufacture.
        Storage: store in a cool, dry place; refrigerate after opening where applicable.
        Certifications: {('Non-GMO Project Verified, ' if rng.random() > 0.4 else '')}Kosher.
        Packaging: recyclable where facilities exist. Net contents vary by pack variant.
    """)
    # consumer reviews (4 per product: mix sentiment)
    for n in range(4):
        if cat == "Beverages" and name == "Aurora Oat Milk" and n < 2:
            sentiment, body = "Negative", NEG_REVIEW["Beverages"][n]
            rating = int(rng.integers(2, 4))
        else:
            if rng.random() < 0.55:
                sentiment = "Positive"
                body = POS_REVIEW[cat][int(rng.integers(0, len(POS_REVIEW[cat])))]
                rating = int(rng.integers(4, 6))
            else:
                sentiment = "Negative"
                body = NEG_REVIEW[cat][int(rng.integers(0, len(NEG_REVIEW[cat])))]
                rating = int(rng.integers(2, 4))
...
# brand guidelines (one per brand)
for brand in sorted({p[1] for p in BASE_PRODUCTS}):
    ...
    add_doc("brand_guideline", f"{brand} Brand Guidelines", brand, cat, f"""
        {brand} Brand Guidelines (NorthStar Brands). Brand positioning: {brand} delivers
        accessible, better-for-you {cat.lower()} for health-conscious households.
        Target consumer: 25-45, urban/suburban, values clean ingredients and transparency.
        Voice & tone: warm, confident, science-backed but approachable. Avoid medical claims.
        Primary palette and logo usage must follow the master kit; never stretch the logo.
        Messaging pillars: clean ingredients, everyday value, sustainability of packaging.
        Mandatory claims review: all on-pack claims require Regulatory + Legal sign-off.
    """)

# trade promo playbook (strategic docs)
add_doc("promo_playbook", "Trade Promotion Playbook — TPR Best Practices", "NorthStar", "All", """
    NorthStar Brands Trade Promotion Playbook: Temporary Price Reductions (TPR). Recommended TPR
    depth is 15-25% off everyday price; depths above 30% rarely improve ROI and erode base price.
    Target a minimum promoted ROI of 0.0 (break-even on incremental margin vs trade spend);
    flag any event with negative ROI for post-event review. Avoid back-to-back TPRs on the same
    SKU for more than 2 consecutive periods to prevent pantry-loading and base erosion.
""")
add_doc("promo_playbook", "Trade Promotion Playbook — Display & Feature Strategy", "NorthStar", "All", """
    ...
    70-180%) and should be reserved for key value items and seasonal peaks. Secure end-cap
    displays at Mass and Club for summer beverage programs and Q4 snacking. Always pair feature
    ads with adequate inventory: confirm weeks-of-supply >= 4 before committing to a display event.
""")
add_doc("promo_playbook", "Trade Promotion Playbook — BOGO Guidelines", "NorthStar", "All", """
    ...
    competitive defense. Require Finance approval when projected ROI is below -0.2. Prefer
    BOGO 50% over full BOGO to protect margin while still signaling value.
""")
...
# competitive briefs (per category)
...
add_doc("competitive_brief", "Competitive Brief — Beverages Category", "NorthStar", "Beverages", """
    ...
    caffeine and zero sugar against legacy energy brands. Cascade Sparkling Water targets the
    no-sweetener segment. Threat: heavy promotional intensity from national beverage players.
""")
add_doc("competitive_brief", "Competitive Brief — Personal Care Category", "NorthStar", "Personal Care", """
    ...
    are positioned for the naturals shopper at Drug and eCommerce. Evergreen Toothpaste competes
    on SLS-free fluoride protection. Opportunity: subscription on Amazon. Threat: dermatologist-
    recommended legacy brands with larger media budgets.
""")

documents = pl.DataFrame(doc_rows).with_columns(pl.col("last_updated").str.to_date())
print(f"documents: {documents.height} rows  (types: "
      f"{documents['doc_type'].value_counts().to_dict()})")

# ---------------------------------------------------------------------------
# Write to Unity Catalog via Databricks Connect bridge
# ---------------------------------------------------------------------------
...
for tname, df in TABLES.items():
    ...
    (sdf.write.format("delta").mode("overwrite")
        ...
        .saveAsTable(f"{CATALOG}.{SCHEMA}.{tname}"))
    cnt = spark.table(f"{CATALOG}.{SCHEMA}.{tname}").count()
    print(f"  wrote {CATALOG}.{SCHEMA}.{tname}: {cnt:,} rows")
...
print("Enabled Change Data Feed on documents table.")
